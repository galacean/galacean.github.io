...
const capControl = gui.add(controls, 'cap', { butt: LineCap.Butt, round: LineCap.Round, square: LineCap.Square});
const joinControl = gui.add(controls, 'join', { bevel: LineJoin.Bevel, round: LineJoin.Round, miter: LineJoin.Miter});
...
engine.sceneManager.activeScene.background.solidColor = new Color(0.8, 0.8, 0.8, 1);
...
.then(data => {
  const lineEntity = rootEntity.createChild("Line");
	...
	for (let index = 0; index < data.features.length; index++) {
		...
		line.points = geometry.coordinates[0].map(p => { return {x: (p[0] - 116.4) * 30, y: (p[1] - 39.9) * 30}});
