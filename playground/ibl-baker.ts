...
WebGLEngine.create({ canvas: "canvas" }).then((engine) => {
  ...
  Promise.all([
    ...
    const bakedLDRCubeMap = IBLBaker.fromTextureCubeMap(
      ldrCubeMap,
      DecodeMode.Gamma
    );
    ...
    SphericalHarmonics3Baker.fromTextureCubeMap(
      hdrCubeMap,
      DecodeMode.RGBE,
      sh
    );
  ...
  function debugIBL(
    bakedLDRCubeMap: TextureCube,
    bakedHDRCubeMap: TextureCube
  ) {
    Shader.create(
      "ibl debug test",
      `
    ...
    gui
      .add(state, "mipLevel", 0, debugTexture.mipmapCount - 1, 1)
