...
渲染导出配置可以用来控制项目的渲染效果和性能等参数。在渲染导出配置中，你可以选择导出的渲染器类型、渲染器参数、后处理效果等等。在导出渲染器时，你可以选择是否启用 SSAO、Bloom、HDR 等效果。

### 物理导出配置

物理导出配置可以用来控制项目的物理效果和性能等参数。在物理导出配置中，你可以选择需要的物理引擎类型、物理参数、碰撞体形状等等。在导出物理参数时，你可以选择是否启用重力、摩擦力、弹力等效果。
