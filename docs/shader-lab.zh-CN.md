...
`ShaderLab` 是一种特别设计用于 Galacean 引擎的 Shader 语言，它可以使用特定的指令对引擎渲染状态、渲染管线进行设置，使多 Pass Shader 的编写更加方便，此外你还可以使用[GLSL](https://www.khronos.org/files/opengles_shading_language.pdf)语言编写渲染管线中的顶点(Vertex)、片元(Fragment)着色器程序，值得一提的是在 `ShaderLab` 中只需声明一次 uniform、attribute 和 varying 变量，引引擎会自动剔除未被着色器程序使用的变量。相比以往手动代码指定的方式，使用 `ShaderLab` 可以更方便、快捷地编写自定义材质的 Shader。
